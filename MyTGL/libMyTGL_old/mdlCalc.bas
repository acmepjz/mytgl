...
'TODO: remove all With statement to improve speed

#Const Release = True
#Const FlatOperatorEnabled = True
#Const CloudOperatorEnabled = True
#Const GradientOperatorEnabled = True
#Const Gradient2OperatorEnabled = True
#Const CellOperatorEnabled = True
#Const NoiseOperatorEnabled = True
#Const BrickOperatorEnabled = True
#Const PerlinOperatorEnabled = True
#Const ImportOperatorEnabled = True
#Const IFSPOperatorEnabled = True
#Const LSystemOperatorEnabled = True
#Const SlowGrowOperatorEnabled = True
#Const RectOperatorEnabled = True
#Const PixelsOperatorEnabled = True
#Const GlowRectOperatorEnabled = True
#Const CrackOperatorEnabled = True
#Const BlurOperatorEnabled = True
#Const ColorOperatorEnabled = True
#Const RangeOperatorEnabled = True
#Const HSCBOperatorEnabled = True
#Const NormalsOperatorEnabled = True
#Const ColorBalanceOperatorEnabled = True
#Const RotZoomOperatorEnabled = True
#Const RotateMulOperatorEnabled = True
#Const SharpenOperatorEnabled = True
#Const DialectOperatorEnabled = True
#Const DistortOperatorEnabled = True
#Const BumpOperatorEnabled = True
#Const AddOperatorEnabled = True
#Const MaskOperatorEnabled = True
#Const ParticleOperatorEnabled = True
#Const SegmentOperatorEnabled = True
#Const BulgeOperatorEnabled = True
#Const TwirlOperatorEnabled = True
#Const UnwrapOperatorEnabled = True
#Const AbnormalsOperatorEnabled = True
...
Public Type cAlphaDibSection
 Width As Long
 Height As Long
 DIBSectionBitsPtr As Long
 b() As Byte
End Type
...
Private Const M_TWO_PI As Double = M_PI * 2
Private Const M_1_2_PI As Double = M_PI / 2
Private Const M_3_2_PI As Double = M_PI * 3 / 2
Private Const M_1_4_PI As Double = M_PI / 4
...
' bmOut.Create w, -h
'End If
If w * h <> bmOut.Width * bmOut.Height Then
 ReDim bmOut.b(w * h * 4& - 1)
 bmOut.DIBSectionBitsPtr = VarPtr(bmOut.b(0))
End If
bmOut.Width = w
bmOut.Height = h
Select Case nOpType
...
'////////////////////////////////
#End If
#If SlowGrowOperatorEnabled Then
Case 11 'SlowGrow
 pCalcSlowGrow bmOut.DIBSectionBitsPtr, bmIn(0).lpbm, w, h, bProps
#End If
#If LSystemOperatorEnabled Then
Case 12 'L-system
 'TODO
 'd.hdc = bmOut.hdc
 'd.Width = w
...
For i = 0 To nCount - 1
 With bmIn(i)
  ...
  pTestBitmap = pTestBitmap And .Width > 0 And .Height > 0 And .lpbm <> 0
 End With
Next i
End Function
#End If
...
For j = 1 To nYSize
 ...
 For i = 1 To nXSize
  ...
  nNum0 = nNum0 + nNumDelta0
  '   if actualNumber>=(nSqrt+¦Å)^2
  '=> if nSqrt^2+nNum>=nSqrt^2+2*nSqrt*¦Å+¦Å^2
  '=> if         nNum>=¦Å(2*nSqrt+¦Å)
  Do Until nNum0 < lp _
...
Const TheConst As Double = M_PI / 512
Dim bDib() As RGBQUAD
Dim tSA As SAFEARRAY2D
...
Const TheConst As Double = M_PI / 1024
Dim bDib() As RGBQUAD
Dim tSA As SAFEARRAY2D
Dim i As Long, j As Long, lp As Long
...
Else
 'init angle
 fAngle = (fAngle - Int(fAngle)) * M_TWO_PI
...
'
'\alpha: is a shininess constant for this material, which decides how "evenly" light is
...
'
'We further define lights as the set of all light sources, \mathbf{L} is the direction vector from the point on the
'surface toward each light source, \mathbf{N} is the normal at this point of the surface, \mathbf{R} is the direction that a
'perfectly reflected ray of light (represented as a vector) would take from this point of the surface, and
'\mathbf{V} is the direction towards the viewer (such as a virtual camera).
'Then the shade value for each surface point Ip is calculated using this equation, which is the Phong
'reflection model:
'
'I_p=K_a*I_a+\sum(k_d*(\mathbf{L}\cdot\mathbf{N})*i_d + k_s*((\mathbf{R}\cdot\mathbf{V})^\alpha)*i_s)
'///////////////////////////////////////Blinn¨CPhong shading model
'If we instead calculate a halfway vector between the viewer and light-source vectors
'     \mathbf{L}+\mathbf{V}
'\mathbf{H}= -------
'    |\mathbf{L}+\mathbf{V}|
'we can replace \mathbf{R}\cdot\mathbf{V} with \mathbf{N}\cdot\mathbf{H}, where \mathbf{N} is the normalized surface normal.
...
For nIndex = 1 To nCount
 'get spin
 f(0) = M_PI * cUnk.fRnd2Float(&HF00D&, nIndex, TheSeed) * f(5)
 'get size
...
'calc table
For i = 0 To 4095
 fAngle = f(0) * M_TWO_PI * ((4095& - i) / 4096#) ^ f(1) '??????
...
Case 1 'normal->polar
 'init ATAN2
 Const TheConst As Double = M_PI / 32768
 For i = 0 To 4095
  TheSin(i) = Atn(i / (4096# - i)) / TheConst '2M_PI -> 65536
 Next i
 'calc
 y = h - 1 'y*2
 yy = y * y
...
Const TheConst As Single = M_PI / 512#
Const TheConst2 As Single = M_PI / 255#
Dim bDib() As RGBQUAD
Dim tSA As SAFEARRAY2D
Dim bDibMap() As RGBQUAD
Dim tSAMap As SAFEARRAY2D
...
pQuatFromRotation r, r.w * -M_PI, r.x, r.y, r.z
r0 = r
'calc
For i = 0 To m - 1
 With bDib(i)
