...
Public Declare Function LZMACompress_API Lib "LZMA.dll" Alias "LzmaCompress" (ByRef dest As Any, ByRef destLen As Long, ByRef src As Any, ByVal srcLen As Long, _
...
Public Declare Function LZMADecompress_API Lib "LZMA.dll" Alias "LzmaUncompress" (ByRef dest As Any, ByRef destLen As Long, ByRef src As Any, ByRef srcLen As Long, _
...
nSize = m + m \ 16 + 4096&
ReDim b(nSize - 1)
ret = LZMACompress_API(b(0), nSize, TheData(lps), m, p, 5, nLevel)
nSize = nSize + 5
ReDim TheDataOut(nSize - 1)
CopyMemory TheDataOut(0), p, 5
CopyMemory TheDataOut(5), b(0), nSize - 5
Erase b
LZMACompress_Simple = ret = 0
End Function
...
m = UBound(TheData) - lps + 1
On Error GoTo 0
If Err.Number <> 0 Or m <= 5 Then
 Erase TheDataOut
 LZMADecompress_Simple = True
 Exit Function
End If
'/////
CopyMemory p, TheData(lps), 5
m = m - 5
ReDim TheDataOut(nSize - 1)
ret = LZMADecompress_API(TheDataOut(0), nSize, TheData(lps + 5), m, p, 5)
LZMADecompress_Simple = ret = 0
End Function
